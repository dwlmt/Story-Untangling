...
class RnnSeqDecoder(SeqDecoder):
    ...
    def _init_decoder_state(self, state: Dict[str, torch.Tensor], final_encoder_output: torch.Tensor = None) -> Dict[
        str, torch.Tensor]:
        """
        ...
        if final_encoder_output is None:
    ...
    def forward(self,
                ...
                final_encoder_output: torch.Tensor = None):

        state = encoder_out

        if target_tokens:
            state = self._init_decoder_state(encoder_out, final_encoder_output)
